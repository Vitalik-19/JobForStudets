...
class LoginViewModel(val database: UserDatabaseDao, application: Application) : AndroidViewModel(application) {
    ...
    private val _editLogin = MutableLiveData<String>()
    val editLogin: MutableLiveData<String>
        get() = _editLogin
    ...
    private fun initializeCreateEmployer(login: String) {
        uiScope.launch {
            _openEmployer.value = getEmployerFromDatabase(login)
        }
    }
    private fun initializeCreateSeeker(login: String) {
        uiScope.launch {
            _openSeeker.value = getSeekerFromDatabase(login)
        }
    }

    fun onLogin() {
        initializeCreateEmployer(_editLogin.value!!)
        if(openEmployer.value?.loginEmployer != null)
            _employerNavigationEvent.value = true
        initializeCreateSeeker(_editLogin.value!!)
